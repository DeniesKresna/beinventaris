...
func RoomIndex(c *gin.Context) {
	...
	Configs.DB.Model(Models.Room{}).Scopes(FilterModel(search, Models.Room{})).Count(&count)
	...
	p, _ := (&PConfig{
		...
		Path:    c.FullPath(),
		Sort:    "id desc",
	}).Paginate(Configs.DB.Preload("Updater").Scopes(FilterModel(search, Models.Room{})), &rooms, count)
